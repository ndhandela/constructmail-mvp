...
async def _require_feature_flag_if_numeric(conn, user_id: str, feature_key: str):
    ...
    if user_id and str(user_id).isdigit():
        await require_feature_flag(conn, int(user_id), feature_key)
...
async def analyze(req: AnalyzeRequest):
    ...
    async with pool.acquire() as conn:
        await require_feature_flag(conn, req.userId, "clash")
...
async def save_assignment(req: AssignmentRequest):
    pool = await get_pool()
    async with pool.acquire() as conn:
        await _require_feature_flag_if_numeric(conn, req.userId, "clash")
